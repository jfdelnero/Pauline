...
static int listen_ports = 0;
static struct ws_events cli_events[MAX_LISTEN_PORTS];
//END
/**
 * @brief Client socks.
 */
...
//STRT Multi ports support patch
static int geteventsidxfromport(int port)
{
	int i;

	i = 0;
	while( i < MAX_LISTEN_PORTS && i < listen_ports )
	{

		if(cli_events[i].port_in == port )
		{
			return i;
		}
		i++;
...
//STRT Multi ports support patch
	int evtsidx = geteventsidxfromport(wfd->client->port_in);

	if( evtsidx >= 0)
		cli_events[evtsidx].onopen(wfd->client);
//END
...
//STRT Multi ports support patch
			int evtsidx = geteventsidxfromport(wfd.client->port_in);
			if( evtsidx >= 0)
				cli_events[evtsidx].onmessage(client, wfd.msg, wfd.frame_size, wfd.frame_type);
//END
		}
...
//STRT Multi ports support patch
	int evtsidx = geteventsidxfromport(wfd.client->port_in);
	if( evtsidx >= 0)
		cli_events[evtsidx].onclose(client);
//END
...
//STRT Multi ports support patch
				lenaddr = sizeof(addr);
				getsockname(sock, (struct sockaddr*)&addr, &lenaddr);

				client_socks[i].port_in = ntohs(addr.sin_port);
//END
...
//STRT Multi ports support patch
	if ( listen_ports >= MAX_LISTEN_PORTS )
		panic("Max listen port reached !");
//END
	...
	/* Copy events. */
//STRT Multi ports support patch
	memcpy(&cli_events[listen_ports], evs, sizeof(struct ws_events));
	cli_events[listen_ports].port_in = port;
	listen_ports++;
//END
...
//STRT Multi ports support patch
	memcpy(&cli_events[listen_ports], evs, sizeof(struct ws_events));
	cli_events[listen_ports].port_in = -1;
	listen_ports++;
//END
